...
export async function POST(request) {
    try {
        ...
        // 2. الاتصال بقاعدة البيانات الآمن والمحمي بالكاش
        await connectToMongo();

        // 3. التحقق من عدم تكرار الحساب (البحث صاروخي بسبب الفهرسة و الـ lowercase التلقائي)
        const userExists = await User.findOne({ email });
        if (userExists) {
            return NextResponse.json({ success: false, message: "المستخدم موجود بالفعل" }, { status: 400 });
        }

        // 4. إنشاء المستخدم (الإصلاح 2: تسمية المتغير بحروف صغيرة user لمنع التعارض مع الموديل)
