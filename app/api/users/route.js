...
export async function GET(request) {
    try {
        ...
        // 3. جلب جميع المستخدمين بدون حقل الباسورد المرتبين بالأحدث أولاً
        const users = await User.find({})
            .select("-password")
            .sort({ createdAt: -1 })
            .lean();
        ...
        console.error("❌ خطأ أثناء جلب المستخدمين للأدمن: ", err);
...
export async function PUT(request) {
    try {
        ...
        if (!auth || (auth.role !== "super_admin" && auth.role !== "admin")) {
            return NextResponse.json({ success: false, message: "غير مصرح لك بتعديل الأدوار" }, { status: 403 });
        }
        ...
        if (auth.role === "admin" && role === "super_admin") {
            return NextResponse.json({ success: false, message: "لا يمكنك تعيين مستخدم كسوبر أدمن" }, { status: 403 });
        }

        await connectToMongo();
        
        // منع الأدمن العادي من تعديل السوبر أدمن الحالي
        if (auth.role === "admin") {
             const targetUser = await User.findById(userId);
             if (targetUser && targetUser.role === "super_admin") {
                 return NextResponse.json({ success: false, message: "لا يمكنك تعديل صلاحيات السوبر أدمن" }, { status: 403 });
             }
        }
        ...
        console.error("❌ خطأ أثناء تحديث رتبة المستخدم: ", err);
        return NextResponse.json({ success: false, message: "حدث خطأ في الخادم الداخلي" }, { status: 500 });
    }
}
